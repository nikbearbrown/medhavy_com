...
function resolve(slug: string): string | null {
  ...
  const filePath = join(process.cwd(), 'public', 'ai1', 'visualizations', `${slug}.html`)
  return existsSync(filePath) ? `/ai1/visualizations/${slug}.html` : null
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const src = resolve(slug)
  if (!src) return { title: 'Visualization - AI+1 - Medhavy' }
  const meta = readHtmlMetaLight(join(process.cwd(), 'public', src))
  return { title: `${meta.title || 'Visualization'} - AI+1 - Medhavy`, description: meta.description || undefined }
}
...
export default async function VisualizationPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const meta = readHtmlMetaLight(join(process.cwd(), 'public', src))
  const title = meta.title || 'Visualization'
      ...
      <div className="w-full border-b bg-background">
        <div className="container px-4 md:px-6 mx-auto py-4 flex items-center justify-between gap-4">
          <div className="min-w-0">
            <Link href="/ai1/visualizations" className="text-sm text-muted-foreground hover:text-foreground mb-1 inline-block">← Back to Visualizations</Link>
