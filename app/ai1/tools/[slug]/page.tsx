...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (doc) return { title: `${doc.title} - Tools - Medhavy`, description: doc.description || doc.title }
  try {
    const rows = await sql`SELECT name, description FROM tools WHERE slug = ${slug}`
    if (rows.length > 0) return { title: `${rows[0].name} - Tools - Medhavy`, description: rows[0].description || rows[0].name }
  } catch {}
      ...
      <div className="flex flex-col w-full" style={{ minHeight: 'calc(100vh - 4rem)' }}>
        <div className="w-full border-b bg-background">
          <div className="container px-4 md:px-6 mx-auto py-4">
            <Link href="/ai1/tools" className="text-sm text-muted-foreground hover:text-foreground mb-1 inline-block">← Back to Tools</Link>
