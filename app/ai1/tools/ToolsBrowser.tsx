      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {filtered.map((tool) => {
            ...
            const href = isArtifact ? `/ai1/tools/${tool.slug}` : tool.claude_url
            const cardContent = (
              <Card className="h-full transition-shadow hover:shadow-md">
                <CardHeader>
                  <CardTitle className="text-lg flex flex-wrap items-center gap-2">
                    {tool.name}
                    {isArtifact && <Badge variant="default" className="text-xs">Artifact</Badge>}
                  </CardTitle>
                  {tool.description && <CardDescription>{tool.description}</CardDescription>}
                </CardHeader>
                <CardContent>
                  {tool.tags && tool.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">{tool.tags.map((tag) => (<Badge key={tag} variant="secondary" className="text-xs">{tag}</Badge>))}</div>
                  )}
                </CardContent>
              </Card>
            )
