...
export default function IllustraeBrowser({ groups }: { groups: Group[] }) {
    ...
    <>
      ...
      {totalDocs === 0 ? (
        <p className="text-muted-foreground">
          {query || activeTag ? 'No previz match your search.' : 'No previz yet.'}
        </p>
      ) : (
          ...
          {filteredGroups.map(g => (
            <section key={g.folder}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {g.docs.map(doc => (
                  <Link key={doc.slug} href={`/illustrae/${doc.slug}`}>
